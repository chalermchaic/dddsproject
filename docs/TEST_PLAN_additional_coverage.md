...
> **บริบท:** ก่อน implement ตาม `plan1v2.md` (dfderd) ได้ตรวจสอบชุดทดสอบที่มีอยู่จริงในโค้ด (`tests/`, `analytics/`, `pages/`, `db/schema.sql`) เทียบกับ 4 กลุ่มที่เคยเสนอไว้ (Unit test DS / Regression 2 บั๊ก / ขยาย E2E ครบโมดูล / RBAC+Index) พบว่า **3 ใน 4 กลุ่มมีอยู่แล้วจริงในโค้ด** (เอกสารแผนงานฝั่ง `dfderd` เขียนสรุปไว้กว้างๆ ว่า "ผ่าน 100%" แต่ไม่ได้ระบุรายละเอียดระดับนี้) เอกสารนี้จึงเน้นเฉพาะช่องว่างที่ตรวจสอบแล้วว่า **ยังไม่มีจริง** เท่านั้น เพื่อไม่ให้เสียเวลาทำซ้ำ
...
เหตุผล: Data Science/ML/AI คือ **Focus หลักของโครงงาน** (ตาม `plan_1_project_report.md`) แต่ยังไม่มีเทสต์ยืนยัน "ความถูกต้องของสูตร/ตรรกะ" โดยตรง — เทสต์กลุ่มนี้ยืนยันด้วย input ที่กำหนดเองแล้วรู้ผลลัพธ์ที่ถูกต้องแน่นอน (ไม่ต้องพึ่งฐานข้อมูลจริง):
...
- บันทึกผลลัพธ์เป็น `docs/INDEX_QUERY_PLAN_REPORT.md` — ใช้อ้างอิงเสริมในหัวข้อ 3.6 ของรายงาน (`plan_1_project_report.md`)
...
อย่าเพิ่มรายการต่อไปนี้ใน `plan1v2.md` หรือแผนงานอื่น เพราะมีอยู่แล้วและผ่านจริง — ตรวจสอบซ้ำได้ด้วยคำสั่ง `pytest tests/ -v` (ไม่รวม `tests/e2e/`):
...
## 4. ผลกระทบต่อรายงาน (`plan_1_project_report.md` / `crm-report.docx`)

เมื่อเทสต์ในหัวข้อ 2.1–2.2 เสร็จและผ่านแล้ว ให้เพิ่มบรรทัดในหัวข้อ 3.9 (ผลการทดสอบระบบ) ของรายงานเวอร์ชันถัดไป (`crm-reportv2.x.docx` ตามกฎใน `AGENTS.md`) ว่ามี unit test ระดับสูตรคำนวณของ Data Science และ E2E จริงของฟอร์มออกใบเสนอราคาเพิ่มเข้ามา เพื่อเพิ่มน้ำหนักให้ Focus 1 (Data Science/ML/AI) มีหลักฐานทดสอบที่ลึกกว่าการรันแล้วไม่พัง
